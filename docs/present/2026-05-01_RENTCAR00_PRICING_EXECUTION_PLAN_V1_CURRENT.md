...
2. 허브 기준값 1개에서 주중/주말 24시간 및 단기 구간 금액을 파생하는 구조를 도입한다.
3. 15일/30일 장기 계산을 IMS monthly source 와 맞는 구조로 정리한다.
4. 공식 저장 구조와 실사용 금액 저장 구조를 분리한다.
...
- 일대여 주중/주말 판정만 교체
- 시간대여는 1차 범위에서 기존 유지
- 장기는 daily policy fallback 이 아니라 monthly source / monthly 공식 기준으로 분리

### 3-2. 저장 원칙
- 기존 `price_policies` 는 실사용 금액 저장소로 유지
- 신규 공식 테이블은 계산 근거 저장소로 추가
- 조회 경로는 필요 시 공식으로 재계산 가능하되, 1차는 기존 실사용 필드와 호환 우선
...
4. monthly source 와 formula 값 차이 표시
...
종료 조건:
- 허브에서 공식 저장과 산출 검증 가능

### Phase 5. 장기 source 정리
목적:
- monthly source 와 formula 관계 정리
...
3. 기존 `price_policies` 금액과 공식 산출값 차이가 날 수 있음
4. monthly source 와 formula source 우선순위 충돌 가능
...
**1차는 정책 잠금과 DB/계산기 분리를 먼저 하고, 그 다음 실제 포함 과금일 기준 계산기로 홈페이지를 교체하며, 장기와 허브는 source 분리 원칙을 유지한 채 단계적으로 연결하는 것이 가장 안전하다.**
