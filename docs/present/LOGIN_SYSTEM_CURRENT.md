...
### 구현 전 확인사항
- Vercel 로그인과 프로젝트 링크 상태 확인
- Supabase env 키 존재 여부 확인
- `booking_orders` 실제 접근 가능 여부 확인
- `profiles` 테이블 및 `booking_orders.user_id` 실제 존재 여부 확인
- 빌드 통과 여부 확인

### 현재 확인된 후속 체크 포인트
- `profiles` 가 없으면 회원 기본정보 upsert 가 실패한다.
- `booking_orders.user_id` 가 없으면 회원 예약내역 API 가 실패한다.
- `/reservations` 가 placeholder 면 로그인 이후 흐름이 끊긴다.
...
- 메일 링크 진입 후 `/reset-password` 화면에서 비밀번호 변경 가능

## Phase 5 검증
- 헤더 상태 반영
- 로그인/로그아웃 UI 동작
- 예약내역 진입 경로 정상
- 로그인 시 `예약내역/로그아웃` 노출, 비로그인 시 `로그인/회원가입` 노출
