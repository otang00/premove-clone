...
## 3. 인증 주입 방식
현재 검증된 인증 방식은 브라우저 로그인 세션 기반 Authorization 헤더 캡처다.

초기 기준:
- 환경변수 `IMS_AUTHORIZATION` 을 워커에 주입해 사용
- 값은 로그인된 브라우저 세션에서 캡처한 Authorization 문자열

보조 기준:
- 필요 시 별도 Playwright 갱신 스크립트로 토큰/헤더 재수집 가능
- 그러나 Phase 4에서는 토큰 자동 갱신까지 구현 범위에 넣지 않음

즉:
- Phase 4의 워커는 **이미 확보된 Authorization 값 사용** 기준으로 잠근다.
