...
export default function GuestBookingsPage() {
  ...
  const canSubmit = useMemo(
    () => Boolean(String(reservationNumber).trim()) && /^\d{10,11}$/.test(normalizePhone(customerPhone)),
    [reservationNumber, customerPhone],
  )

  const handleLookup = () => {
    const reservation = findGuestReservation({ reservationNumber, customerPhone })

    if (!reservation) {
      setResult(null)
      setError('일치하는 비회원 예약을 찾지 못했습니다. 예약번호와 휴대폰번호를 다시 확인해 주세요.')
      return
    }
  ...
  const handleCancel = () => {
    ...
    const cancelled = cancelGuestReservation({ reservationNumber: result.reservationNumber, customerPhone })
    setResult(cancelled)
  }
        ...
        <div className="container detail-layout" style={{ paddingTop: 24, paddingBottom: 24 }}>
          <article className="detail-card panel" style={{ display: 'grid', gap: 16 }}>
            <div>
              ...
              <p className="small-note" style={{ marginTop: 8 }}>예약번호와 휴대폰번호로 예약 상태를 확인하고 취소할 수 있습니다.</p>
            </div>
